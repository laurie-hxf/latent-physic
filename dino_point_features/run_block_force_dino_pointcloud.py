...
from dino_point_features.projection import DinoFeatureProjector, camera_observations_from_frames
...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "outputs" / "mujoco_dino_point_features" / "block_force"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--object", action="append", default=None, help="Object name/prompt. Repeat for multiple objects.")
    parser.add_argument(
    ...
    parser.add_argument("--device", type=str, default=None, help="Device for DINO and segmentation models.")
    ...
    parser.add_argument(
        ...
        help="DINO backbone. Use dinov3_vitl16 with local PointWorld DINOv3 weights for closest parity.",
    )
    parser.add_argument("--selected-layers", type=str, default=None, help="Comma/space-separated layer ids, or default.")
    ...
    parser.add_argument(
        "--no-depth-fallback",
        action="store_true",
        help="Disable nearest-depth fallback for points that miss the strict depth threshold in every camera.",
    )
...
def feature_and_write_cloud(
    *,
    ...
    ply_path = frame_dir / f"{stem}.ply"
    npz_path = frame_dir / f"{stem}_dino_features.npz"
    write_ascii_ply(ply_path, cloud, comments=comments)
    point_features = projector.project_feature_map(cloud.points, observations, feature_map)
    save_point_feature_npz(
        ...
        metadata={
            **feature_metadata,
            "source_ply": str(ply_path.name),
        },
    )
    visible = int(np.count_nonzero(point_features.visibility_counts > 0))
    fallback = int(np.count_nonzero(point_features.depth_fallback_used))
    return {
        "ply": str(ply_path),
        "npz": str(npz_path),
        "point_count": int(len(cloud)),
        "feature_dim": int(point_features.features.shape[-1]),
        "visible_feature_count": visible,
        "depth_fallback_count": fallback,
        "assigned_feature_count": int(visible + fallback),
    }
...
def capture_and_write(
    ...
    observations = camera_observations_from_frames(capture.camera_frames)
    feature_map = extractor.encode_images([obs.rgb for obs in observations])
    frame_summary: dict[str, object] = {"frame_index": int(frame_index), "clouds": {}}

    frame_summary["clouds"]["merged_segments"] = feature_and_write_cloud(
        frame_dir=frame_dir,
        stem="merged_segments",
        cloud=capture.merged_cloud,
    ...
    for segment_name, cloud in capture.segment_clouds.items():
        stem = _safe_name(segment_name)
        frame_summary["clouds"][stem] = feature_and_write_cloud(
            frame_dir=frame_dir,
            stem=stem,
            cloud=cloud,
            observations=observations,
            feature_map=feature_map,
            projector=projector,
            feature_metadata={**feature_metadata, "segment_name": segment_name},
    ...
    return [track.to_json_dict() for track in capture.tracks], frame_summary
...
def main() -> None:
    ...
    feature_metadata = {
        ...
        "reference": "PointWorld-style projective DINO feature sampling",
    }
    ...
    try:
        for step in range(args.num_steps + 1):
            if step % args.frame_stride == 0:
                tracks, frame_summary = capture_and_write(
                    args,
                    pipeline,
                    step,
                    extractor,
                ...
                merged = frame_summary["clouds"]["merged_segments"]
                print(
                    f"frame {step}: points={merged['point_count']} "
                    f"features={merged['assigned_feature_count']} "
                    f"strict={merged['visible_feature_count']} "
                    f"fallback={merged['depth_fallback_count']} "
                    f"dim={merged['feature_dim']}"
                )
    ...
    print(f"wrote DINO point-feature outputs to {args.output_dir.resolve()}")
